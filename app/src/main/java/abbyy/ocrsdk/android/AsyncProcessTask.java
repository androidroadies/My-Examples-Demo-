...
public class AsyncProcessTask extends AsyncTask<String, String, Boolean> {
	...
	protected Boolean doInBackground(String... args) {
		...
		try {
			...
			restClient.applicationId = "wedoAppsScan";
			// You should get e-mail from ABBYY Cloud OCR SDK service with the
			// application password
			restClient.password = "BRxAKeuPMI3hDBEw/6kQ2vVN";
			...
			// processingSettings.setOutputFormat(
			// ProcessingSettings.OutputFormat.txt );
			...
			Task task = restClient.processBusinessCard(inputFile,
					busCardSettings);
			...
			while (task.isTaskActive()) {
				...
				// performance.
				// Note: if your application queues several files and waits for
				// them
