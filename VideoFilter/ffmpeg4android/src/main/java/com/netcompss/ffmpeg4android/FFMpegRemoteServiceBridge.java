	...
	private IFfmpgefRemoteServiceBridge.Stub myRemoteServiceStub = new IFfmpgefRemoteServiceBridge.Stub() {
		...
		public void setFfmpegCommand(String command) throws RemoteException {
			 ...
			 if (command.contains("'") || command.contains("\"")) {
				Log.i(Prefs.TAG, "Command contains special chars, handling...");
				...
				for (int i = 0; i < _command.length; i++) {
					 ...
					 if (_command[i].endsWith("'")) {
						 doMerge = false;
						 _command[mergeStartIndex] = mergedStr;
					 }
				}
				
				/// clean the string array from nulls;
				String[] cleaned = new String[_command.length - cleanedNum] ;
				for (int i = 0; i < _command.length; i++) {
					if (_command[i] != null) 
						cleaned[i] = _command[i];
				}
				//////////////////////////////////////
				_command = cleaned;
		...
		public void runTranscoding() throws RemoteException  {
			Log.i(Prefs.TAG,"=======service runTranscoding() called, setting _run to true======");
				_run = true;
		}
	};
			...
			while (true) {
				if (_command != null && _run) {
					...
					try {
						...
						Log.i(Prefs.TAG, "===============Running command from thread path: " + Prefs.getWorkingFolderForNative());
						vk.run(_command, Prefs.getWorkingFolderForNative());
						_run = false;
					} catch (Exception e) {
